...
use tardis::web::ws_processor::{
    ws_broadcast, ws_echo, TardisWebsocketInstInfo, TardisWebsocketMessage, TardisWebsocketMgrMessage, TardisWebsocketReq, TardisWebsocketResp, WsHooks, WS_SYSTEM_EVENT_AVATAR_ADD, WS_SYSTEM_EVENT_AVATAR_DEL, WS_SYSTEM_EVENT_INFO
};
    ...
    TardisFuns::ws_client("ws://127.0.0.1:8080/ws/dyn/_/true", move |msg| async move {
        ...
        if receive_msg.event == Some(WS_SYSTEM_EVENT_AVATAR_ADD.to_string()) && receive_msg.msg.as_str().unwrap() == "c" {
            ...
            return Some(Message::Text(
                TardisFuns::json.obj_to_string(&receive_msg.into_req(String::new(), json! {"c"}, from_avatar.clone(), Some(vec![from_avatar]))).unwrap(),
            ));
        }
