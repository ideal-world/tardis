...
pub async fn init(cluster_config: &ClusterConfig, web_server_config: &WebServerConfig) -> TardisResult<()> {
    let access_host = web_server_config.access_host.as_ref().unwrap_or_else(|| &web_server_config.host).to_string();
    let access_port = web_server_config.access_port.unwrap_or_else(|| web_server_config.port);
